...
const ChatWrapper = ({ fileId }: ChatWrapperProps) => {
  const { data, isLoading } = trpc.getFileUploadStatus.useQuery(
    ...
    },
    {
      refetchInterval: ({ state }) => {
        return state.data?.status == "SUCCESS" || state.data?.status == "FAILED"
          ? false
          : 500;
      },
    }
  ...
  if (isLoading) {
    return (
      <div className="min-h-full bg-zinc-0 flex divide-y divide-zinc-200 flex-col justify-between gap-2">
